...
top<-as.data.frame(cbind(FeatureIdx=topanfeat,Class=rep("Positive")))
bot<-as.data.frame(cbind(FeatureIdx=botanfeat,Class=rep("Negative")))
...
ipshitsmodel_temp<-unique(ipshitsmodel_temp[,-c(3:14)])
...
#ipshitsmodel$PatSize<-sqrt(ipshitsmodel$FCP*(ipshitsmodel$FeatSize^2))
ipshitsmodel$PatArea<-ipshitsmodel$FCP*(ipshitsmodel$FeatSize^2)
# ipshitsmodel$SPCSize<-sqrt(ipshitsmodel$FeatSize^2-
#                              (ipshitsmodel$FeatSize^2*ipshitsmodel$FCP))
# ipshitsmodel$SumArea<-ipshitsmodel$TriArea*ipshitsmodel$NumTri+
# ipshitsmodel$CircArea*ipshitsmodel$NumCirc+
#   ipshitsmodel$LineArea*ipshitsmodel$NumLine
# # ipshitsmodel$TNP<-ipshitsmodel$NumCirc+ipshitsmodel$NumTri+ipshitsmodel$NumLine
# ipshitsmodel$DiffArea<-ipshitsmodel$SumArea-ipshitsmodel$PatArea
...
# ggplot(ipshitsmodel,aes(WN0.2,SumArea,
...
ggplot(ipshitsmodel,aes(PatArea,WN1,
                        colour=as.factor(Class)))+geom_point()+theme_bw()+
  scale_color_discrete(name ="Surface Class for \n OCT4 expression", 
                       labels=c("Positive","Negative"))+ 
  theme(legend.position = c(.8, .8),
        text = element_text(size=18))+scale_y_continuous(limits=c(0.001, 0.02))+
...
ggplot(ipshitsmodel,aes(DC,WN0.4,
                        colour=as.factor(Class)))+geom_point()+theme_bw()+ geom_density2d()

ggplot(ipshitsmodel,aes(DC,CCDunsc,
                        colour=as.factor(Class)))+geom_point()+theme_bw()+ geom_density2d()
ggplot(ipshitsmodel,aes(DC,WN0.4,
                        colour=as.factor(Class)))+geom_point()+theme_bw()+ geom_density2d()
...
#make nice plot
# library(ggplot2)
# ggplot(ipshitsmodel,aes(FeatSize,WN0.2,
#                         colour=as.factor(Class)))+geom_point()+theme_bw()+
#   scale_color_discrete(name ="Surface Class for \n OCT4 expression", 
#                        labels=c("Positive","Negative"))+ theme(legend.position = c(.8, .8))
# 
